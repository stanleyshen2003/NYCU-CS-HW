...
class evaluation_model():
    def __init__(self, seed=51):
        ...
        self.mode = 'DDPM'
        model_path = 'iclevr_checkpoint_300.pth'
        self.test_path = 'new_test.json'
        ...
        self.generator = DiffusionModel(device=None, dataset_name='iclevr', checkpoint_name=model_path)
        
        labels = labels.to(self.device)
        self.generator.nn_model.eval()

        
        images, intermidiate_samples = self.generator.generate(32, 8, 500, 0.0001, 0.02, epoch='test')
        inter = []
        index = 1
        for i in range(13, len(intermidiate_samples)):
            inter.append(intermidiate_samples[i][index])
        inter.append(images[index])
            
        print(intermidiate_samples[0].shape)
        print(len(intermidiate_samples))
        # print(intermidiate_samples)
        self.generate_grid(inter, denoise=True)
        self.generate_grid(images)

        print(self.eval(images.cuda(), labels.cuda()))
        
    # def generate_images(self, labels):
    #     images, 
    #     return images
    
    def generate_grid(self, images, denoise=False):
        ...
        if not denoise:
            ...
            image_grid.save(f'results/{self.mode}/denoise.png')
