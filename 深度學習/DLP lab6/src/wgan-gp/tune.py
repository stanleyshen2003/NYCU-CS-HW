...
class Agent():
    def __init__(self):
        ...
        self.generator.load_state_dict(torch.load(f'output2/checkpoint/{self.start_epoch}/generator.pth'))
        self.discriminator.load_state_dict(torch.load(f'output2/checkpoint/{self.start_epoch}/discriminator.pth'))
        
        self.criterion = torch.nn.BCELoss()
        self.optimizer_G = torch.optim.Adam(self.generator.ln1.parameters(), lr=lr_G, betas=betas_G)
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs+1):
            ...
            with tqdm(total=len(train_dataset), ncols=120) as pbar:
                for i, (images, labels) in enumerate(train_loader):
                    ...
                    noise = torch.randn(images.size(0), 64).to(self.device)
                    ...
                    # classifier_loss = self.criterion(aux_output, labels).mean()
                    ...
                    if i % 1 == 0:
                        ...
                        class_result = self.classifer(fake_data)
                        aux_errG = self.criterion(class_result.view(-1), labels.view(-1)).mean()
                        gen_cost = -gen_cost.mean()
                        g_cost =  aux_errG# * epoch / self.epochsgen_cost +
                        total_loss_C += aux_errG.item()
                        total_loss_G += gen_cost.item()
                        g_cost.backward()
                        self.optimizer_G.step()
                    pbar.set_postfix({ "wdist":w_dist.item(), "gloss":gen_cost.item(), "closs_G":aux_errG.item()}) 
