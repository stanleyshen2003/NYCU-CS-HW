...
Action MCTS_agent::decide_inipos(GameState& state){
    ...
    for(int i = 0; i < 15; i++){
        for(int j = 0; j < 15; j++){
            int count = 0;
            if(state.user_state[i][j] == '1'){
                for(int k = 0; k < 4; k++){
                    int new_x = i + state.directions4[k][0], new_y = j + state.directions4[k][1];
                    if(new_x >= 0 && new_x < 15 && new_y >= 0 && new_y < 15 && state.user_state[new_x][new_y] == '0'){
                        count++;
                    }
                }
                if(count > 0){
                    ...
                    count = 0;
                    for(int k = 0; k < 8; k++){
                        int new_x = i + state.directions8[k][0], new_y = j + state.directions8[k][1];
                        if(new_x >= 0 && new_x < 15 && new_y >= 0 && new_y < 15 && state.user_state[new_x][new_y] == '1'){
                            count++;
                        }
                    }
                    if(count > maxdir){
